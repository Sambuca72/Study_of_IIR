...
int main(int argc, char const *argv[]) {
    ...
    #pragma acc data copy(previousMatrix[0:matrixSize * matrixSize], updatedMatrix[0:matrixSize * matrixSize])
    {
        while (iteration < maxIterations && error > accuracy) {
            #pragma acc parallel loop independent collapse(2) vector vector_length(16) gang num_gangs(40) present(previousMatrix, updatedMatrix)
            ...
            if ((iteration + 1) % 10000 == 0) {
                error = 0.0;
                #pragma acc parallel loop independent collapse(2) vector vector_length(16) gang num_gangs(40) reduction(max:error) present(previousMatrix, updatedMatrix)
                for (size_t i = 1; i < matrixSize - 1; i++) {
                    for (size_t j = 1; j < matrixSize - 1; j++) {
